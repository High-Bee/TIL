...
incheon_1618 <- read.csv("C:/semiPj/data/incheon_1618.csv",
                       header = TRUE,
                       encoding = "CP949") # 기상 데이터 로드

decal_1618 <- read.csv("C:/semiPj/data/기상으로인한사유-지연및결항.csv",
                  fill = T,
                  header = T,
                  encoding = "CP949",
                  stringsAsFactors = F)

indc_1618 <- read.csv("C:/semiPj/data/indc1618_2.csv",
                     header = TRUE,
                     encoding = "CP949") # 기상 데이터 로드
...
dc1618_df$일자 <- as.Date(dc1618_df$일자)
dc1618_df[,4:19] <- sapply(dc1618_df[,4:19], as.numeric) # 데이터 타입 가공
indc1618[,13:15] <- sapply(indc1618[,13:15], as.numeric)
...
dc1618[,4:19] <- transform(dc1618[,4:19], as.numeric(dc1618[,4:19]))


dc1618 <- as.numeric(dc1618[,4:19])



dc1618[,4:19] <- ifelse(dc1618[,4:19] == "0",0,as.numeric(dc1618[,4:19]))
head(dc1618)
...
in1618day <- read.csv("C:/semiPj/data/daymean_nona - 복사본.csv",
                      header = TRUE,
                      encoding = "CP949")
...
dc1618_df <- rename(dc1618_df, year=일자)
